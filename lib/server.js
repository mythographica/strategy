...
const strategy_types_1 = require("./strategy-types");
const log_socket_1 = require("./log-socket");
...
async function executeCommand(filePath, args) {
    ...
    const g = global;
    const store = g.StrategyMCP;
    const runtime = store?.get(exports.StoreMeta);
...
class StrategyServer {
    constructor() {
        ...
        const g = global;
        if (!g.StrategyMCP) {
            g.StrategyMCP = new Map();
        }
        const store = g.StrategyMCP;
        if (!store.has(exports.StoreMeta)) {
            const runtime = new strategy_types_1.StrategyRuntime('1.0');
            store.set(exports.StoreMeta, runtime);
        }
    ...
    async run() {
        ...
        if (logPort > 0) {
            const actualPort = await (0, log_socket_1.startLogSocket)(logPort);
            (0, logger_1.logInfo)(`Strategy log socket listening on 127.0.0.1:${actualPort}`);
        }
        const transport = new stdio_js_1.StdioServerTransport();
        await this.server.connect(transport);
        (0, logger_1.logInfo)('Strategy MCP server running on stdio');
    }
}
